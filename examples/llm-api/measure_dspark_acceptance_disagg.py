...
def _summarize_counters(context: dict, generation: dict, num_requests: int, draft_len: int) -> dict:
    ...
    if any(context["drafted"].values()) or any(context["accepted"].values()):
        raise RuntimeError(
            "Unexpected drafting on the context worker; DSpark must run on decode only"
        )
    ...
    positions = drafted.keys() | accepted.keys()
